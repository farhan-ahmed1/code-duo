...
test.describe('Real-time collaboration', () => {
  test('two users see each other\'s edits in real time', async ({ browser }) => {
    ...
    await expect(pageB.locator('.monaco-editor')).toContainText('Hello from A', {
      timeout: 2000,
    });
  ...
  test('concurrent edits from both users converge', async ({ browser }) => {
    ...
    await expect(pageA.locator('.monaco-editor')).toContainText('AAA', { timeout: 3000 });
    await expect(pageA.locator('.monaco-editor')).toContainText('BBB', { timeout: 3000 });
  ...
  test('offline edits sync when reconnected', async ({ browser }) => {
    ...
    await expect(pageA.locator('.monaco-editor')).toContainText('Offline edit', {
      timeout: 5000,
    });
